...
function dntheme_paging_nav() {
    ...
    $links = paginate_links( array(
        ...
        'prev_text' => __( '<i class="fa fa-angle-left" aria-hidden="true"></i>', 'twentyfourteen' ),
        'next_text' => __( '<i class="fa fa-angle-right" aria-hidden="true"></i>', 'twentyfourteen' ),
    ) );
...
function related_category_fix($value)
{
    ...
    if( $query->have_posts() ):?>
        <div class="related__post <?php echo $post_type; ?>">
            <header class="related__post--header">
                <h3 class="related__post--title"><?php echo $related_title ?></h3>
            </header>
            <?php  ?>
...
function dntheme_get_wmpl($value='')
{
    if(in_array('wpml-translation-management/plugin.php', apply_filters('active_plugins', get_option('active_plugins')))){
        ...
        echo '<div class="qtranslate__current d-flex">';
        echo "<img src='".get_theme_file_uri('assets/flag/'.$currentLang.'.svg')."' width='".$flag_width."' height='".$flag_height."' >";
        echo '</div>';

        echo do_shortcode('[wpml_language_switcher type="menu" flags=1 native=1 translated=1][/wpml_language_switcher]');
    }else{
